...
!    print*, "Inside Function: ",hcor, A, b ,R ! debug
  ...
  real(kr) FUNCTION  GrTTfd(R,R0,i,j)
    ...
    real(kr) :: a,b0,s,TT,GR,bmix

    CALL read_parameters(b0,a,s)

!    if (debug) write(fiit(dampdbg),'(2A3,2X,7(X,F15.5))') coords(i)%atom_type, coords(j)%atom_type, b0,bmix,basym(b0,polar(i),Ni(i),Zaim(i)),Ni(i),Zaim(i),polar(i),Ni(j),Zaim(j),polar(j)
