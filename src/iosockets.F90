...
contains
  ...
  subroutine ASSERT(must_be_true)
    implicit none   
    LOGICAL ::  must_be_true
    if ( .not. must_be_true ) STOP 'Assert error in module iosockets'
  ...
  subroutine socketCreate(self, nAtom, host, verbosity, protocol, port)
    ...
    CALL ASSERT(.not.associated(self))

!     INITALLOCATE_P(self)
    NULLIFY(self)
    ALLOCATE(self)    

    CALL ASSERT(.not.self%tInit)
    
    CALL ASSERT(natom > 0)
    CALL ASSERT(verbosity >= 0)
  ...
  subroutine socketRetrieve(self, coord, cell, rid)
    ...
    real(kr8), intent(out)        :: coord(:,:) ! Atomic coordinates
    real(kr8), intent(out)        :: cell(3,3)  ! Lattice vectors
    
    character(len=IPI_MSGLEN)    :: header
    integer :: nAtom
    
    ! single precision in the communication interface for some reason
    real(kr8), allocatable :: commsBuffer1(:)
    real(kr8)              :: commsBuffer2(9)
    integer :: commsBufferI
    character(len=:), allocatable :: commsBufferC

    logical :: isinit = .false.
    CALL ASSERT(associated(self))
    
    CALL ASSERT(self%tInit)
    CALL ASSERT(size(coord,dim=1) == 3)
...
!     ALLOCATE_(commsBuffer1, (self%nAtom*3))
    ALLOCATE(commsBuffer1(self%nAtom*3))
    ...
    if (trim(header) /= "POSDATA") then
      STOP 'coordsFromSocket: Unexpected message from server'
    end if
...
!     DEALLOCATE_(commsBuffer1)
  ...
  subroutine socketSend(self, energy, forces, stress)
    type(socketData), pointer :: self
    real(kr8), intent(in) :: energy            ! Total Energy
    real(kr8), intent(in) :: forces(:,:)   ! Atomic Forces
    real(kr8), intent(in) :: stress(3,3)       ! Stress Tensor
    ...
    CALL ASSERT(self%tInit)
    CALL ASSERT(size(forces,dim=1) == 3)
    ...
    if (trim(header)/='GETFORCE') then
      STOP 'forcesToSocket: Error in socket communication!'
    end if
